...
char code_to_char(uint8_t x) {
    if (x <= 25) {
        ...
        //dbg_printf(DBG_INFO, "ERROR1: %i IS OUT OF RANGE 0-63 FOR BASE64 ENCODING\n", x);
        printf("ERROR1: %i IS OUT OF RANGE 0-63 FOR BASE64 ENCODING\n", x);
        exit(EXIT_FAILURE);
...
uint8_t char_to_code(char x) {
    if ((x >= 'A') && (x <= 'Z')) {
        ...
        //dbg_printf(DBG_INFO, "ERROR2: %c (0x%x) IS INVALID CHARACTER FOR BASE64 DECODING\n", x, x);
        printf("ERROR2: %c (0x%x) IS INVALID CHARACTER FOR BASE64 DECODING\n", x, x);
        // exit(EXIT_FAILURE);
    } //TODO: improve error management
}
...
int bin_to_b64_nopad(const uint8_t * in, int size, char * out, int max_len) {
    ...
    /* check input values */
    if ((out == NULL) || (in == NULL)) {
        //dbg_printf(DBG_INFO, "ERROR3: NULL POINTER AS OUTPUT IN BIN_TO_B64\n");
        printf("ERROR3: NULL POINTER AS OUTPUT IN BIN_TO_B64\n");
        return -1;
    }
    if (size == 0) {
        *out = 0; /* null string */
    ...
    switch (last_bytes) {
        ...
        case 2: /* 2 bytes left to encode -> +3 chars */
            ...
            break;
        default:
            //dbg_printf(DBG_INFO, "switch default that should not be possible");
    ...
    if (max_len < (result_len + 1)) { /* 1 char added for string terminator */
        //dbg_printf(DBG_INFO, "ERROR: OUTPUT BUFFER TOO SMALL IN BIN_TO_B64\n");
...
int b64_to_bin_nopad(const char * in, int size, uint8_t * out, int max_len) {
    ...
    /* check input values */
    if ((out == NULL) || (in == NULL)) {
        //dbg_printf(DBG_INFO, "ERROR4: NULL POINTER AS OUTPUT OR INPUT IN B64_TO_BIN\n");
        printf("ERROR4: NULL POINTER AS OUTPUT OR INPUT IN B64_TO_BIN\n");
        return -1;
    }
    ...
    switch (last_chars) {
        ...
        case 1: /* only 1 char left is an error */
            //dbg_printf(DBG_INFO, "ERROR5: ONLY ONE CHAR LEFT IN B64_TO_BIN\n");
            printf("ERROR5: ONLY ONE CHAR LEFT IN B64_TO_BIN\n");
            return -1;
    ...
    if (max_len < result_len) {
        //dbg_printf(DBG_INFO, "ERROR6: OUTPUT BUFFER TOO SMALL IN B64_TO_BIN\n");
        printf("ERROR6: OUTPUT BUFFER TOO SMALL IN B64_TO_BIN\n");
        return -1;
    }
    ...
    if (last_bytes == 1) {
        ...
        out[3*i + 0] = (b >> 16) & 0xFF;
        if (((b >> 12) & 0x0F) != 0) {
            //dbg_printf(DBG_INFO, "WARNING: last character contains unusable bits\n");
...
int bin_to_b64(const uint8_t * in, int size, char * out, int max_len)
{
    ...
    switch (ret%4) {
        case 0: /* nothing to do */
            return ret;
        case 1:
            //dbg_printf(DBG_INFO, "ERROR7: INVALID UNPADDED BASE64 STRING\n");
            printf("ERROR7: INVALID UNPADDED BASE64 STRING\n");
            return -1;
        case 2: /* 2 chars in last block, must add 2 padding char */
            if (max_len >= (ret + 2 + 1)) {
                ...
                //dbg_printf(DBG_INFO, "ERROR8: not enough room to add padding in bin_to_b64\n");
                printf("ERROR8: not enough room to add padding in bin_to_b64\n");
                return -1;
            }
        case 3: /* 3 chars in last block, must add 1 padding char */
            if (max_len >= (ret + 1 + 1)) {
                ...
                //dbg_printf(DBG_INFO, "ERROR9: not enough room to add padding in bin_to_b64\n");
                printf("ERROR9: not enough room to add padding in bin_to_b64\n");
                return -1;
            }
        default:
            //dbg_printf(DBG_INFO, "switch default that should not be possible");
...
{
    if (in == NULL) {
        //dbg_printf(DBG_INFO, "ERRORA: NULL POINTER AS OUTPUT OR INPUT IN B64_TO_BIN\n");
        printf("ERRORA: NULL POINTER AS OUTPUT OR INPUT IN B64_TO_BIN\n");
        return -1;
    }
...
{
	int ret;
	memcpy(bin, &default_nv_value, sizeof(CHELSEA_NV_ITEMS));
	dbg_printf(DBG_INFO, "%08X:%d:%08X\r\n", bin, sizeof(CHELSEA_NV_ITEMS), &default_nv_value);
	//uprint_memory32(bin, sizeof(CHELSEA_NV_ITEMS));

	//int bin_to_b64(const uint8_t * in, int size, char * out, int max_len)
	ret = bin_to_b64(bin, sizeof(CHELSEA_NV_ITEMS), base64, 2048);
	dbg_printf(DBG_INFO, "End : %d\r\n", ret);
	base64[ret] = 0;
	dbg_printf(DBG_INFO, "%s\r\n\r\n", base64);
	//int b64_to_bin(const char * in, int size, uint8_t * out, int max_len)
	ret = b64_to_bin(base64, ret, bin, 1024);

	dbg_printf(DBG_INFO, "RET: %d:%d\r\n", ret, memcmp(bin, &default_nv_value, sizeof(CHELSEA_NV_ITEMS)));

	dbg_printf(DBG_INFO, "u32 : %d\r\n", sizeof(u32));
	dbg_printf(DBG_INFO, "u16 : %d\r\n", sizeof(u16));
	dbg_printf(DBG_INFO, "u8  : %d\r\n", sizeof(u8));

	dbg_printf(DBG_INFO, "double : %d\r\n", sizeof(double));
	dbg_printf(DBG_INFO, "PDU : %d\r\n", PHY_PDU_MAX_LENGTH);
	dbg_printf(DBG_INFO, "packet size : %d\r\n", sizeof(MAC_UPLINK_PACKET_TYPE));
}
#endif
